...
/* b8ab — TraceX requires more than this define to link cleanly:
 * FileX (and likely USBX) reference _fx_trace_event_insert /
 * _ux_trace_event_insert which are only compiled into the libraries
 * when FX_ENABLE_EVENT_TRACE / UX_ENABLE_EVENT_TRACE are also defined
 * during the LIBRARY build (not just at app build time). The prebuilt
 * objects in Middlewares/ST/{filex,usbx,threadx} were built without
 * these defines, so enabling TX_ENABLE_EVENT_TRACE here causes
 * "Unknown destination type" linker errors.
 *
 * To enable TraceX properly:
 *   1. Add -DTX_ENABLE_EVENT_TRACE -DFX_ENABLE_EVENT_TRACE
 *      -DUX_ENABLE_EVENT_TRACE to the middleware build flags
 *   2. Rebuild Middlewares/ST/threadx, filex, usbx with those flags
 *   3. Re-enable this define
 *   4. Or: provide weak stubs for the missing _*_trace_event_insert
 *      functions (less invasive but loses USBX/FileX trace events).
 *
 * For now, kept disabled. Trace infrastructure (buffer in PSRAM,
 * tx_trace_enable call in App_ThreadX_Init, diagnostic globals) is
 * ready to use once link errors are resolved. */
