...
Reset_Handler:
  /* Force Thread mode to use MSP regardless of what SPSEL the boot ROM left.
   * If SPSEL=1 (Thread on PSP), "mov sp, r0" would set PSP instead of MSP,
   * leaving MSP corrupt and PSPLIM (from the boot ROM production FSBL) at
   * _estack — causing immediate STKOF on the first exception frame push. */
  mrs   r1, CONTROL
  bic   r1, r1, #0x2    /* clear SPSEL: Thread mode → MSP */
  msr   CONTROL, r1
  isb
  /* Also clear PSPLIM so a stale boot-ROM value cannot trigger STKOF. */
  movs  r1, #0
  msr   PSPLIM, r1
  ldr   r0, =_sstack
  msr   MSPLIM, r0      /* set MSP lower guard */
  ldr   r0, =_estack
  msr   MSP, r0         /* explicitly set MSP (not "sp" which follows SPSEL) */
