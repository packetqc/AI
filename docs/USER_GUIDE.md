# User Guide — Grammar-Driven Tiny LLMs (Local + On-NPU)

> Full reference for the framework. For the quick start, see the [README](../README.md) (entry point: `scripts/nocode_runner.py`).
...
It also has a **nocode** evolution: instead of running a grammar's logic from hand-written CPU code, the working logic is *transposed into trainable tokens, carried inside the model, emitted at inference, and executed by a grammar-agnostic runner* (`scripts/nocode_runner.py`) — so a new or changed grammar needs **no per-grammar CPU code**. See [Nocode — Model-Carried Logic](#nocode--model-carried-logic) and [docs/NOCODE_RUNNER.md](NOCODE_RUNNER.md).
...
- **Composes a verdict** — `INERT` / `INCONCLUSIVE` / `EXECUTABLE-CAPABILITY`, written to `models/forensics/`. Independent analysis code (only `gguf-py` + the Ollama API), generalising across `qwen2` / `llama` / `mistral` GGUFs. Full guide: [docs/MODEL_SECURITY_RE.md](MODEL_SECURITY_RE.md).
...
layers so it converges from what was already learned. Full detail: [docs/NOCODE_RUNNER.md](NOCODE_RUNNER.md).
...
Full reference: **[docs/NOCODE_RUNNER.md § CLI & REPL reference](NOCODE_RUNNER.md#cli--repl-reference)**.
...
(`--arch qwen2|qwen3|llama|mistral` — the same grammar pipeline on any base family; all four build
and **load** in Ollama, with per-arch GGUF export fixes for qwen3 `head_dim` + mistral, plus
`gguf_arch_fix.py` to patch a built model without retraining), and **function arguments +
inter-function data flow** (`<tool> <arg…>` at the prompt; one function gathers data and injects it into the next).
...
**[docs/NOCODE_RUNNER.md](NOCODE_RUNNER.md)**.
...
**Full documentation:** [docs/GRAMMAR_TOOLS.md](GRAMMAR_TOOLS.md)
...
flow in [docs/STM32_NPU_DEPLOYMENT.md](STM32_NPU_DEPLOYMENT.md)):
...
Full end-to-end walkthrough: [docs/STM32_NPU_DEPLOYMENT.md](STM32_NPU_DEPLOYMENT.md).
...
> [docs/STM32_NPU_DEPLOYMENT.md](STM32_NPU_DEPLOYMENT.md).
...
flow: [docs/STM32_NPU_DEPLOYMENT.md](STM32_NPU_DEPLOYMENT.md) § *On-device deployment*.
...
see [docs/NOCODE_RUNNER.md § Security fixtures](NOCODE_RUNNER.md#security-fixtures--capabilities-carried-in-a-model).
...
**Full methodology, tooling, and roadmap:** [docs/MODEL_SECURITY_RE.md](MODEL_SECURITY_RE.md).
